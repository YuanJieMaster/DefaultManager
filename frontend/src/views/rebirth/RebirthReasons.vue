<template>
  <div class="rebirth-reasons-container">
    <!-- 页面标题 -->
    <div class="page-header">
      <h2>重生原因维护</h2>
      <p class="subtitle">管理系统中的重生原因数据</p>
    </div>
    ...
    <el-card class="reasons-table-card">
      ...
      >
        ...
        <el-table-column prop="reasonContent" label="重生原因内容" min-width="200">
    ...
    >
      <el-form
        ...
        class="reason-form"
      >
        <el-form-item label="重生原因内容" prop="reasonContent">
          <el-input
            v-model="formData.reasonContent"
            type="textarea"
            placeholder="请输入重生原因内容"
            rows="3"
            maxlength="200"
            show-word-limit
          />
    ...
    >
      <div class="batch-delete-content">
        ...
        <div class="warning-text">
          <p>您确定要删除选中的 {{ selectedRowKeys.length }} 条重生原因吗？</p>
...
// 重生原因数据
const rebirthReasons = ref<RebirthReasonResponseDTO[]>([])

// 对话框相关
const dialogVisible = ref(false)
const dialogTitle = ref('新增重生原因')
const formRef = ref()
const formData = reactive<RebirthReasonDTO>({
  reasonContent: '',
  isEnabled: true,
  sortOrder: 0
})
...
const formRules = {
  reasonContent: [
    { required: true, message: '请输入重生原因内容', trigger: 'blur' },
...
// 获取重生原因数据
const fetchRebirthReasons = async () => {
  loading.value = true
  try {
    // 获取所有重生原因
    const response = await rebirthReasonApi.getAllRebirthReasons()
    ...
    // 更新表格数据和分页信息
    rebirthReasons.value = paginatedData
...
// 处理表格选中变化
const handleSelectionChange = (selection: RebirthReasonResponseDTO[]) => {
  selectedRowKeys.value = selection.map(item => item.id)
}

// 打开新增对话框
const handleCreate = () => {
  dialogTitle.value = '新增重生原因'
...
// 打开编辑对话框
const handleEdit = (row: RebirthReasonResponseDTO) => {
  dialogTitle.value = '编辑重生原因'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (currentId) {
      // 更新重生原因
      await rebirthReasonApi.updateRebirthReason(currentId, formData)
      ElMessage.success('重生原因更新成功')
    } else {
      // 创建重生原因
      await rebirthReasonApi.createRebirthReason(formData)
      ElMessage.success('重生原因创建成功')
    }
    ...
    if (error !== 'cancel') {
      ElMessage.error(currentId ? '重生原因更新失败' : '重生原因创建失败')
      console.error(currentId ? '更新重生原因失败:' : '创建重生原因失败:', error)
    }
  } finally {
...
const handleStatusChange = async (id: number, isEnabled: boolean) => {
  try {
    await rebirthReasonApi.toggleRebirthReasonStatus(id)
    ElMessage.success('状态切换成功')
  } catch (error) {
    // 恢复原状态
    const reason = rebirthReasons.value.find(r => r.id === id)
    if (reason) {
      reason.isEnabled = !isEnabled
    }
    ElMessage.error('状态切换失败')
    console.error('切换重生原因状态失败:', error)
  }
}
...
const handleDelete = async (id: number) => {
  try {
    ...
    await rebirthReasonApi.deleteRebirthReason(id)
    ElMessage.success('重生原因删除成功')
    fetchRebirthReasons()
  } catch (error) {
    if (error !== 'cancel') {
      ElMessage.error('重生原因删除失败')
      console.error('删除重生原因失败:', error)
    }
  }
}
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    ElMessage.success(`成功删除 ${selectedRowKeys.value.length} 条重生原因`)
    batchDeleteDialogVisible.value = false
    selectedRowKeys.value = []
    fetchRebirthReasons()
  } catch (error) {
    ElMessage.error('批量删除失败')
    console.error('批量删除重生原因失败:', error)
  } finally {
